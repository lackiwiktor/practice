...
public class TeamMatch extends Match {
  ...
  public TeamMatch(
      Ladder ladder, boolean ranked, boolean duel, MatchArenaCopy arena, Team teamA, Team teamB) {
  ...
  public InventorySnapshot createInventorySnapshot(PracticePlayer player) {
    ...
    if (team instanceof SoloTeam && opponent instanceof SoloTeam) {
  ...
  public List<String> getBoard(PracticePlayer player) {
    ...
    if (player.isSpectating()) {
      ...
      if (playerTeam instanceof SoloTeam && opponentTeam instanceof SoloTeam) {
        ...
        lines.add(
            ChatColor.GREEN
                ...
                + ": "
                + ChatColor.WHITE
                + getAlivePlayersCount(playerTeam)
                + "/"
                + playerTeam.size());
        lines.add(
            ChatColor.BLUE
      ...
      if (playerTeam instanceof SoloTeam && opponentTeam instanceof SoloTeam) {
        if (getState() == MatchState.COUNTDOWN) {
        ...
        lines.add(
            ChatColor.WHITE + "Opponent Ping: " + ChatColor.YELLOW + soloTeamOpponent.getPing());
        lines.add(" ");
        ...
        if (ladder.isBridge()) {
          lines.add(
              ChatColor.WHITE
                  + "Your Score: "
                  + ChatColor.YELLOW
                  + playerTeam.getStatistics().getBridgeScore());
          lines.add(
              ChatColor.WHITE
                  + "Opponent Score: "
                  + ChatColor.YELLOW
                  + opponentTeam.getStatistics().getBridgeScore());
        }
        ...
        lines.add(
            ChatColor.RED
                + "Opponents: "
                + ChatColor.WHITE
                + getAlivePlayersCount(opponentTeam)
                + "/"
                + opponentTeam.size());
        lines.add(" ");
      } else {
        ...
        for (PracticePlayer session : playerTeam.getPlayers()) {
          ...
          String line =
              " "
                  + ((!isOnline || !infoTracker.isAlive(session))
                      ? (ChatColor.STRIKETHROUGH + ChatColor.GRAY.toString())
                      : "")
                  ...
                  + (isOnline && infoTracker.isAlive(session)
                      ? (ChatColor.WHITE
                          + " "
                          + FormatUtil.formatHealthWithHeart(session.getHealth()))
                      : "");
          lines.add(line);
        }
