...
import me.ponktacology.snapshot.ChunkSnapshot;
...
public class MatchArenaCopy extends Arena implements Restorable {
  ...
  public void takeSnapshot() {
    forEachChunk(
        chunk ->
            chunkSnapshots.put(LongHash.toLong(chunk.getX(), chunk.getZ()), chunk.takeSnapshot()));
  }
  ...
  public void restore() {
    ...
    chunkSnapshots.forEach(
        (key, value) ->
            world.getChunkAt(LongHash.msw(key), LongHash.lsw(key)).restoreFromSnapshot(value));
