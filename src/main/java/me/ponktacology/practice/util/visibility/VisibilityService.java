...
public class VisibilityService extends Service {
  ...
  public void update(PracticePlayer observer, PracticePlayer observable, Visibility visibility) {
    update(observer, observable, visibility, false);
  }

  public void update(
      PracticePlayer observer, PracticePlayer observable, Visibility visibility, boolean retry) {
    ...
    if (!observer.isOnline() || !observable.isOnline()) {
      ...
      if (!retry) {
        Logger.log("Retrying update of visibility in 1 second.");
        TaskDispatcher.runLater(
            () -> update(observer, observable, visibility, true), 1L, TimeUnit.SECONDS);
      }
      return;
    }
